    ...
    def attack(
        ...
        for step in range(steps):
            ...
            loss = self._objective(
                inputs + delta, labels, temp_targets, temp_fake_labels
            )
    ...
    def _init_optimizer(self, delta_iterable):
        ...
        if "Adam" in self.optim:
            ...
            return torch.optim.SGD(
                delta_iterable, lr=tau_sgd, momentum=0.9, weight_decay=0
            )
        else:
            return torch.optim.SGD(
                delta_iterable, lr=tau_sgd, momentum=0.0, weight_decay=0
            )
...
class WatermarkPoisoning(BaseAttack):
    ...
    def attack(
        ...
        if num_targets == num_inputs:
            ...
            delta = (
                temp_targets.repeat(num_inputs // num_targets, 1, 1, 1)[:num_inputs]
                - inputs
            )
        else:
            factor = num_targets // num_inputs
            delta = (
                temp_targets[: (factor * num_targets)]
...
class AlignmentPoisoning(BaseAttack):
    ...
    def _objective(self, inputs, labels, temp_targets, temp_fake_labels):
        ...
        target_grad = torch.autograd.grad(
            target_loss, differentiable_params, retain_graph=True, create_graph=True
        )
...
class HTBD(BaseAttack):
    ...
    def attack(
        ...
        for step in range(steps):
            ...
            # Gradient step
            loss = self._objective(
                inputs + delta, temp_targets, input_indcs, target_indcs
            )
            (delta.grad,) = torch.autograd.grad(
                loss, delta, retain_graph=False, create_graph=False, only_inputs=True
            )
    ...
    def _apply_patch(self, temp_targets):
        patch_shape = [
            [
                3,
                random.randint(
                    int(0.2 * temp_targets.shape[2]), int(0.4 * temp_targets.shape[2])
                ),
                random.randint(
                    int(0.1 * temp_targets.shape[3]), int(0.2 * temp_targets.shape[3])
                ),
            ]
            for _ in range(temp_targets.shape[0])
        ]
...
class PatchAttack(BaseAttack):
    ...
    def attack(
        ...
        x_locations, y_locations = self._set_locations(
            temp_targets.shape, temp_target_labels, patch_shape
        )
...
class PatchAttackVariant(PatchAttack):
    ...
    def _patch(self, inputs, labels, patches, x_locations, y_locations):
        ...
        for i in range(inputs.shape[0]):
            ...
            outputs[i, :, x : x + xo, y : y + yo] = (
                patches[temp_label] - inputs[i, :, x : x + xo, y : y + yo]
            )
        return outputs
...
class PatchAttackVariantKnownSize(PatchAttack):
    ...
    def attack(
        ...
        patch_shape = [
            [
                inputs.shape[1],
                random.randint(int(0.5 * self.eps), int(1.75 * self.eps)),
                random.randint(int(0.5 * self.eps), int(1.75 * self.eps)),
            ]
            for _ in range(self.num_classes)
        ]
...
class AdaptivePatchAttack(PatchAttackVariant, AlignmentPoisoning):
    ...
    def attack(
        ...
        for step in range(steps):
            optimizer.zero_grad()
            inputs_mask = self._patch(inputs, labels, patches, x_in, y_in)
            targets_mask = self._patch(
                temp_targets, temp_target_labels, patches, x_t, y_t
            )

            patched_inputs = inputs + inputs_mask
            patched_temp_targets = temp_targets + targets_mask
            loss = self._objective(
                patched_inputs, labels, patched_temp_targets, temp_target_labels
            )
            grads = torch.autograd.grad(
                loss, patches, retain_graph=False, create_graph=False, allow_unused=True
            )
            # Optim step
            ...
            with torch.no_grad():
                for patch in patches:
                    patch.data = torch.max(
                        torch.min(patch, (1 - self.dm) / self.ds), -self.dm / self.ds
                    )
...
class AdaptivePatchAttackUnconstrained(AdaptivePatchAttack):
    ...
    def attack(
        ...
        x_t, y_t = self._set_locations(
            temp_targets.shape, temp_target_labels, patch_shape_targets
        )
        ...
        for step in range(steps):
            ...
            targets_mask = self._patch(
                temp_targets, temp_target_labels, patches_target, x_t, y_t
            )
...
class PatchAttackPairs(BaseAttack):
    ...
    def _get_mask(self, inputs):
        ...
        for example in range(inputs.shape[0]):
            x_length = random.randint(
                int(0.1 * inputs.shape[2]), int(0.4 * inputs.shape[2])
            )
            y_length = random.randint(
                int(0.1 * inputs.shape[3]), int(0.4 * inputs.shape[3])
            )
