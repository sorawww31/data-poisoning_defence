...
class WitchHTBD(_Witch):
    def _run_trial(self, victim, kettle):
        ...
        for step in range(self.args.attackiter):
            ...
            for batch, example in enumerate(dataloader):
                ...
                indcs = random.sample(
                    list(range(len(self.temp_targetset))), validated_batch_size
                )
                for i in indcs:
                ...
                loss, prediction = self._batched_step(
                    poison_delta, poison_bounds, example, victim, kettle, targets
                )
                target_losses += loss
    ...
    def _batched_step(
        self, poison_delta, poison_bounds, example, victim, kettle, targets
    ):
