...
class KettleExternal(_Kettle):
    ...
    def __init__(
        ...
        if "xtrain" in data_package.keys():
            ...
            self.trainset, self.validset, self.poisonset, self.targetset = (
    ...
    def _load_metapoison_files(self, data_package):
        ...
        possible_poison_ids = torch.arange(len(data_package["xtrain"]))[
            data_package["ytrain"] == data_package["ytargetadv"]
        ]
        ...
        targetset = torch.utils.data.TensorDataset(
            data_package["xtarget"],
            torch.as_tensor(data_package["ytargetadv"]),
            torch.arange(len(data_package["xtarget"])),
        )

        self.poison_setup = dict(
            poison_budget=self.args.budget,
            target_num=len(data_package["xtarget"]),
            poison_class=data_package["ytargetadv"][0],
            target_class=data_package["ytarget"][0],
